...
if [[ -z $FARGATE_VERSION ]]; then
	...
	echo "Define Task Name"
	echo "Example: export TASK_NAME=ms-subscripciones-microservice-task"
	exit -1
	...
	echo "Example: export SERVICE_NAME=ms-subscripciones-microservice"
	exit -1
...
if [[ $RESULT -gt 0 ]]; then
  ...
  export SERVICE_ARN=$SERVICE_ARN
	...
	aws ecs create-service --cli-input-json file:///deploy/deploy/ecs/initial-service.json --service-registries registryArn=${SERVICE_ARN},port=8080
fi
